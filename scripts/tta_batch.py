import csv, json, subprocess, os, sys, time, argparse
...
def run(model, arch, in_csv, out_csv, thresh, img_size, resume=False, flush_every=10, timeout=90):
    ...
    # Prepare writer (append or write header)
    write_header = True
    if resume and os.path.exists(out_csv):
        write_header = False
    ...
    for i, row in enumerate(rows, 1):
        ...
        if not img or not os.path.exists(img):
            errs += 1
            if i % 20 == 0: print(f"  [{i}/{n}] missing: {img}", flush=True)
            continue
        if img in done:
            continue
        ...
        try:
            res = subprocess.run(
                [sys.executable, os.path.join(os.path.dirname(__file__), "tta_infer.py"),
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    run(args.model, args.arch, args["in_csv"] if hasattr(args, "__getitem__") else args.in_csv,
        args["out_csv"] if hasattr(args, "__getitem__") else args.out_csv,
